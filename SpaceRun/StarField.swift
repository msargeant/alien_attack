...
class StarField: SKNode {
  ...
  func initSetup() {
    ...
    // Because we need to call a method on self from inside a code block, we must create a weak reference to self this is what we are doing with the weak self constant.
    
    // Why? The run action holds a strong reference to the code block and the node (self) holds a strong reference to the action. Now, if the code block held a strong reference to the node (self), then the action, the code block, and the node would form a retain cycle and would never get deallocated => memory leak.
  ...
  func launchStar() {
    ...
    if let scene = self.scene {
      ...
      // Move the star toward the bottom of the screen using a random duration between 0.1 and 1 second removing te star when it passes the bottom edge.
      
      // The different speeds (based on duration) will give us the illusion of a parallax effect.
      
      let destY = 0.0 - scene.size.height - star.size.height
      
      let duration = Double(-star.alpha + 3.0)
